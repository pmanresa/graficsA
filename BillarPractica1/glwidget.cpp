...
void GLWidget::newBola()
{
    ...
    obj = new Bola(0.0, 0.03, -0.5, 0.03, "0");
    newObjecte(obj);
}
...
{
    newPlaBase();
    capsaPla = esc->plaBase->calculCapsa3D();

    newBola();
    capsaBola = esc->bola->calculCapsa3D();

    newConjuntBoles();
    for (int i=0; i<esc->conjuntBoles->boles.size(); i++) {
        CapsesBoles.push_back(esc->conjuntBoles->boles[i]->calculCapsa3D());
    }
}
